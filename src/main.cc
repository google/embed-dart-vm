...
namespace dart {
  ...
  Dart_Handle LoadScript(
      const char* script, bool resolve, Dart_Handle core_library);

  Dart_Handle LibraryTagHandler(
      Dart_LibraryTag tag, Dart_Handle library, Dart_Handle url) {
  ...
  Isolate* CreateIsolate(const char* script, const char* main, bool resolve,
    ...
    std::cout << __FUNCTION__ << ": " << script << ", " << main << ", " <<
      resolve << ", " << data << ", " << *error << std::endl;
    Dart_Isolate isolate =
      Dart_CreateIsolate(script, main, snapshot_buffer, NULL, error);
    assert(isolate);
...
*/
  ...
  Dart_Handle FilePathFromUri(Dart_Handle script, Dart_Handle core_library) {
    Dart_Handle args[2] = { script, Dart_False() };
    return Dart_Invoke(
        core_library, Dart_NewString("_filePathFromUri"), 2, args);
  }
  ...
  Dart_Handle LoadScript(const char* script,
    ...
    if (resolve) {
      ...
      if (Dart_IsError(resolved_script)) return resolved_script;
    } else {
